...
def parse_arguments():
    ...
    parser.add_argument(
        '--port', type=str, help="the port you are listening on. eg. 25540", required=True)
...
def check_block_in_chain(blockchain, block_header_hash):
    # cleaned_keys will not have the most updated chain if resolve not called here
    blockchain.resolve()
...
if __name__ == '__main__':
    ...
    p = Process(target=start_mining, args=(
        block_queue, transaction_queue, public_key, private_key,))
    p.start()
